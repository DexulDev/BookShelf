...
{
    public Estudiante(String nombre, String apellido, String numeroTelefono, String correoElectronico, String direccion){
        super(nombre, apellido, numeroTelefono, correoElectronico, direccion);
        credencial = new CredencialEstudiante(nombre, apellido, numeroTelefono, correoElectronico, direccion, diaActual());
    }

    public void solicitarPrestamo(String fechaInicio, String fechaFinal, String nombreLibro) {
        PrestamoEstudiante prestamo = new PrestamoEstudiante(fechaInicio, fechaFinal, nombreLibro);
        agregarPrestamo(prestamo);
    }

    public void renovarCredencial(){
        ((CredencialEstudiante)credencial).renovar();
    }
    ...
    public boolean solicitarLibro(Contenido libro){
        try{
            PrestamoEstudiante PrestamoTemporal = new PrestamoEstudiante(diaActual(), fechaFinal(diaActual()), libro.getTitulo());
            //1. Verificar vigencia credencial
            //2. Verificar que el libro esté disponible
            //3. Verificar que el estudiante no tenga adeudos
    ...
    public boolean solicitarRevista(Revista revista){
        try{
            PrestamoEstudiante PrestamoTemporal = new PrestamoEstudiante(diaActual(), fechaFinal(diaActual()), revista.getTitulo());
            //1. Verificar vigencia credencial
            //2. Verificar que el libro esté disponible
            //3. Verificar que el estudiante no tenga adeudos
            //4. Si todo está bien, solicitar el préstamo
    ...
    public boolean solicitarTesis(Tesis tesis){
        try{
            PrestamoEstudiante PrestamoTemporal = new PrestamoEstudiante(diaActual(), fechaFinal(diaActual()), tesis.getTitulo());
            //1. Verificar vigencia credencial
            //2. Verificar que el libro esté disponible
            //3. Verificar que el estudiante no tenga adeudos
            //4. Si todo está bien, solicitar el préstamo
